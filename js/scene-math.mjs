...
export function getPanoramaOffset(elapsedSeconds, durationSeconds) {
...
export const phaseProgress = (progress, start, end) =>
  clamp01((progress - start) / (end - start));

export function interpolatePoint3(collapsed, expanded, progress) {
...
export function getWavyLinkPoint(from, to, t, link, elapsedSeconds) {
...
export function getResponsiveMode(width, reducedMotion, webglAvailable) {
  if (width <= 767 || reducedMotion || !webglAvailable) return 'static';
...
export function mapSceneState(progress, viewportWidth) {
  ...
  return {
    ...
    copyOpacity: normalized >= 0.27 ? 0 : 1 - copyExit,
    copyY: -32 * copyExit,
