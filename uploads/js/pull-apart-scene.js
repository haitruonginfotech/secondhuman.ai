...
class PullApartScene {
  ...
  createIntroLinks() {
    ...
    const material = () => new THREE.LineDashedMaterial({
      color: 0xffffff, transparent: true, opacity: 0,
      dashSize: 0.016, gapSize: 0.028, depthWrite: false,
    });
    for (let slot = 0; slot < INTRO_LINK_SLOTS; slot += 1) {
      ...
      this.introLinks.push({ line, travel, travelMaterial, travel2, travelMaterial2, slot, cycleSeconds: 3.2, offset: slot * 0.4 });
    }
  }
  ...
  renderFrame(now) {
    ...
    this.nodes.forEach((node, nodeIndex) => {
      if (p < 0.44) {
        const t = (elapsed + node.pingPhase * INTRO_CYCLE) / INTRO_CYCLE;
        const epoch = Math.floor(t);
        const local = t - epoch;
        const roll = seededRandom(0xab1e + nodeIndex * 7919 + epoch * 104729);
        let px; let py; let guard = 0;
        do {
          px = (roll() - 0.5) * 3.6;
          py = -0.28 + (roll() - 0.5) * 1.9;
          guard += 1;
        } while (Math.abs(px) < 1.15 && Math.abs(py + 0.1) < 0.55 && guard < 8);
        node.scatter.set(px, py, 0.15 + roll() * 0.4);
        node.introOn = local < 0.5 + roll() * 0.12 ? 1 : 0;
      } else {
        node.introOn = 1;
      }
      ...
      node.group.position.copy(node.scatter);
      ...
      const driftAmp = node.driftAmp * (1 - boxForm) * nodeVisN;
      ...
      node.labelMaterial.opacity = 0.95 * vis * (1 - labelFade);
    ...
    this.introLinks.forEach(({ line, travel, travelMaterial, travel2, travelMaterial2, slot, cycleSeconds, offset }) => {
      ...
      const pc = this.nodes[c].group.position;
      ...
      const SEG = 16;
      const swing = Math.sin(elapsed * 5.4 + slot * 1.7);
      ...
      line.computeLineDistances();
      line.material.opacity = 0.3 * active * linkVis;
      ...
      ride(travel, travelMaterial, clamp01((phase - 0.04) / 0.46), 1);
      ride(travel2, travelMaterial2, clamp01((phase - 0.04) / 0.46 - 0.15), 0.9);
    });
