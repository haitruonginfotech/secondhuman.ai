...
function initWhoTilt() {
  ...
  visual.addEventListener('pointerleave', leave, { passive: true });
  cleanups.push(() => {
...
function initCardTilt() {
  // Light pointer-follow tilt on the stack cards
...
function initSpotlights() {
  // Cursor-following grid spotlight (CSS vars --mx/--my) on marked sections.
...
function initFallbackOrbit() {
  ...
  const nodes = orbitLabels.map((label, index) => {
    ...
    courier.setAttribute('fill', '#8fb4ff');
  ...
  if (!reducedMotion.matches) {
    ...
    const onMove = (event) => {
      if (orbit.dragging) {
        ...
        orbit.hoverTY = (((event.clientX - rect.left) / rect.width) * 2 - 1) * 7;
        orbit.hoverTX = -(((event.clientY - rect.top) / rect.height) * 2 - 1) * 4;
      }
      orbitKick();
    };
